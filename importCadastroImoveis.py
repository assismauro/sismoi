...
def createTable(cursor):
    ...
    cursor.execute('''CREATE TABLE "public"."cadastroimoveis" ( 
        ...
        SELECT AddGeometryColumn ('cadastroimoveis','geom',4326,'POINT',2);
    ''')
    ...
    for line in infile:
        try:
            ...
            if (sLat != '') or (sLong != ''):
                ...
                lat=dms2dd(sLat)
                long=dms2dd(sLong)
                sql='INSERT INTO cadastroimoveis(uf,geom) VALUES ({2},ST_SetSRID(ST_MakePoint({0}, {1}), 4326))'.format(long,lat,uf)
                cur.execute(sql)
                if (j % 50000) == 0:
                    conn.commit()
                j+=1
